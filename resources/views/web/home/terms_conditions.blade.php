@extends('web.template.main')

@section('content')

    <div class="container-fluid">
        <div class="row">
            <div class="col-12 text-center my-5">
                <h2 class="color-primary">Aviso de privacidad </h2>

            </div>
        </div>

        <div class="row">
            <div class="col-12">
                <p class="color-gray">Rigunes tiene la necesidad de ofrecerte la seguridad y calidez que tú te mereces, por tal motivo nos hemos dado a la tarea de establecer normativas y reglas que,
                    como usuario al participar y utilizar este servicio, constituirá la aceptación de este acuerdo.</p>

                <p class="color-gray">Rigunes es una marca de BIGTECHSOLUTIONS S.A.S. de C.V. con domicilio Calle Palma número exterior 111 número interior colonia Tlacopa localidad s.n. Lorenzo Tepaltitlan municipio Toluca entidad federativa México.</p>

                <p class="color-gray">Nuestro sitio web con dominio www.rigunes.com.mx cuenta con un certificado que nos permite mantener segura una conexión a Internet, así como para proteger cualquier información confidencial que se envíe. Tanto el hosting como el diseño y desarrollo de Rigunes es proporcionado por BIGTECHSOLUTIONS S.A.S. de C.V. Si desea ponerse en contacto con nosotros el correo electrónico es el siguiente contacto@bigtechsolutions.mx.</p>

                <p class="color-primary"><b>Términos y condiciones de uso</b></p>

                <p class="color-gray">Rigunes es una marca registrada por BIGTECHSOLUTIONS la cual es una empresa constituida legalmente por las normas mexicanas.</p>
                <p class="color-gray">Hacemos todo lo posible para que la información este actualizada y completa, a pesar de nuestro mejor esfuerzo, en nuestro sitio las especificaciones de los productos que Rigunes ofrece puede estar sujeta a cambios sin previo aviso.</p>
                <p class="color-gray">Rigunes declara que los productos que se ofrecen en nuestra plataforma no son de nuestra propiedad, sino directamente de los proveedores de calzado, siendo así estos pueden estar sujetos a normas de propiedad intelectual.</p>
                <p class="color-gray">Rigunes se reserva todos los derechos no expresamente otorgados bajo el presente documento.</p>
                <p class="color-gray">Todos los procesos, código, diseño, animaciones, imágenes, fotos y contenido son propiedad de BIGTECHSOLUTIONS S.A.S. de C.V.</p>
                <p class="color-primary"><b>Política de compra</b></p>

                <p class="color-gray">En Rigunes queremos que disfrutes tanto de hacer compras como de vender el calzado que te ofrecemos, así que elaboramos cuidadosamente las aplicaciones que ocuparas en tu día a día para que le saques el máximo provecho a tu tiempo y esfuerzo. A continuación, te presentamos la política de compra siguiendo el flujo de las actividades que tendrás que realizar para hacer una compra, seguido de las indicaciones y exclusiones en caso de querer hacer un cambio de producto, una garantía respecto al calzado y si lo deseas un reembolso.</p>

                <p class="color-gray" >Cabe mencionar que no se requiere estar registrado para realizar compras, pero los usuarios registrados cuentan con privilegios, descuentos y promociones en algunas de sus compras, uno de los beneficios es contar con herramientas para la administración de su negocio.</p>
                <p class="color-gray">Rigunes ofrece solo los productos que los administradores del sitio hayan publicado, este producto contara con una descripción e imagen correspondiente, el valor del producto no es negociable y se respetara el precio, oferta o descuento que el producto tenga a la fecha de la compra si por algún motivo fuese modificado.</p>
                <p class="color-gray">No aplica para promociones, descuentos o precios que fueron actualizados posteriormente a una compra.</p>
                <p class="color-gray">El precio del producto es en moneda nacional (MXN).</p>
                <p class="color-gray">El precio excluye impuestos, gastos de envío, seguro, salvo que se señale explícitamente lo contrario.</p>
                <p class="color-gray">Posteriormente tendrá que realizar el pago correspondiente a la orden de compra a través de las diversas maneras que Rigunes le ofrece.</p>
                <p class="color-gray">Ya realizada la selección de productos que desea adquirir, se irán abonando a un carrito de compras digital para su posterior compra. Una vez aceptando el pedido del producto, se generará una orden de compra con los datos correspondientes a la compra realizada la cual le será enviada a su correo electrónico.</p>
                <p class="color-gray">En caso de tener algún problema con esta orden de compra o de no recibirla en un lapso de 24 horas se deberá hacer lo siguiente. En el caso de estar registrado verifique en su cuenta de usuario el estatus de su orden de compra, de manera contraria notifique al correo de la empresa contacto@rigunes.com.mx.</p>
                <p class="color-gray">Nos importa demasiado que no haya ningún problema respecto al cargo que se te aplica, por tal motivo le solicitamos que si observa algún cargo indebido o fraudulento en la tarjeta o cuenta bancaria con la que realizo la compra nos notifique al siguiente correo contacto@rigunes.com.mx para evitar cargos inoportunos en su cuenta.</p>
                <p class="color-gray">Una vez abonado el pago del producto, se procederá a realizar el envió del producto, el cual el usuario de nuestra plataforma deberá cubrir. Salvo que la compra cuente con envió gratis en su descripción.</p>
                <p class="color-gray">A partir de este momento, el cliente contara con la fecha aproximada de entrega, cabe mencionar que Rigunes cuenta con un servicio externo para el envió de nuestros productos, por lo que todos estos datos se los proporcionara nuestra plataforma como intermediario.</p>

                <p class="color-gray">No se podrá modificar la dirección de envió una vez realizado el pago del producto, por lo que se le pide al usuario verificar la dirección de envió antes de realizar el pago.</p>

                <p class="color-gray">En caso de no contar con la entrega en el plazo de las fechas aproximadas, deberá comunicarse directamente a contacto@rigunes.com.mx, para verificar el estatus de su entrega. El cliente no podrá reclamar ningún tipo de interés a la empresa.</p>
                <p class="color-primary"><b>Promociones y descuentos</b></p>
                <p class="color-gray">Las promociones, descuentos y ofertas serán de valides si aún se encuentran publicadas en el sitio. Estas serán sujetas a condiciones, restricciones al tiempo que tengan de vigencia o cantidad de productos, mismas que serán sujetas a cambios sin ningún previo aviso.</p>
                <p class="color-primary"><b>Cambio de producto</b></p>
                <p class="color-gray">Rigunes ofrece la posibilidad de realizar el cambio del producto únicamente si es por talla y color, no se podrá realizar el cambio por otro modelo ya que como lo mencionamos antes en este documento Rigunes tiene más de un proveedor de calzado.</p>
                <p class="color-gray">El cambio se realizará a través de la plataforma del usuario, siguiendo los pasos correspondientes que se publicaran en el sitio.</p>
                <p class="color-gray">Rigunes se deslinda de cualquier responsabilidad si el cambio de producto no aplica por los términos y condiciones del distribuidor, de cualquier manera, se notificará ya sea en la cuenta de usuario o al correo electrónico que el cliente haya registrado.</p>
                <p class="color-primary"><b>Garantia del producto</b></p>
                <p class="color-gray">Nuestro producto es elaborado de manera tradicional e incluso artesanalmente, por lo que, en algunos casos, el calzado puede diferir al de la foto publicada en nuestro sitio. Rigunes realiza un gran proceso de verificación y calidad para garantizar que el usuario reciba el producto sin ningún detalle.</p>
                <p class="color-gray">Por tal motivo la garantía solo podrá ser aplicada si el calzado cuenta con algún defecto de fabricación y deberá ser notificada inmediatamente al correo contacto@rigunes.com.mx donde será verificado su caso en particular.

                </p>


                <p class="color-gray" >Posteriormente se le darán a conocer los pasos a seguir según el tipo de garantía que aplique a su producto.</p>
                <p class="color-gray" >Rigunes se esfuerza a lo máximo por sus clientes, por tal motivo le recomendamos a nuestros clientes seguir el proceso de garantía al pie de la letra, ya que de otra forma quedara exento a aplicar la garantía.</p>
                <p class="color-gray">Cabe mencionar que, si el producto ya ha sido abierto, usado, maltratado o cambiado, este será excluido de cualquier garantía o reembolso.</p>

                <p class="color-gray">Cabe mencionar que, si el producto ya ha sido abierto, usado, maltratado o cambiado, este será excluido de cualquier garantía o reembolso.</p>

                <p class="color-gray">Rigunes ofrece la oportunidad de reembolzar el dinero que has invertido en nuestros productos en caso de que no estés completamente satisfecho con el servicio.</p>
                <p class="color-gray">Una vez que hayas solicitado el reembolso nosotros te enviaremos a tu correo electrónico los pasos a seguir para realizar este procedimiento o puedes comunicarte al siguiente correo: contacto@rigunes.com.mx.</p>
                <p class="color-gray">Cabe mencionar que, si el producto ya ha sido abierto, usado, maltratado o cambiado, este será excluido de cualquier garantía o reembolso.</p>
                <p class="color-gray">El reembolso al que será acreedor el usuario insatisfecho con el producto será deducido únicamente al precio del producto, no se hará la devolución por cualquier pago de envío u otros gastos.</p>
                <p class="color-gray">Para aplicar el reembolso tendrá que pagar el envió del producto hasta nuestra sucursal con la dirección al principio mencionada para hacer la valoración correspondiente. Una vez concluido este proceso se le proporcionara el reembolso correspondiente.</p>
                <p class="color-gray">Rigunes se reserva todos los derechos no expresamente otorgados bajo el presente documento.</p>
            </div>

        </div>

    </div>

@endsection
